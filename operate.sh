...
## chang here
SERVICE_DIR=./
SERVICE_NAME=guoya-mini-app-server-0.0.1-SNAPSHOT
...
case "$1" in 
	start)
		...
		then
			...
			exec nohup /usr/bin/java -Xms128m -Xmx512m -jar ${SERVICE_DIR}/${SERVICE_NAME}\.jar --spring.profiles.active=${SPRING_PROFILES_ACTIVE} >./logs/guoya-teach.log 2>&1 &
			echo "start success"
		else
