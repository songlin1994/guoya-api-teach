...
public class SkuController {
  ...
  //  @Validated : 校验 后面的实体bean 是否满足Validated的规则 (如果不加注解 不启用 校验规则)
    ...
    return RespUtils.getBase(20000,"添加成功",null);
  }
  ...
  @Min(value = 1, message = "pageSize必须是大于等于1的整数") int pageSize) {
    List<TProdSku> tProdSkuList = skuSvc.getAllSKU(pageNum, pageSize);
    return RespUtils.getBase(20000,"查询成功",tProdSkuList);
  }

  public static void main(String[] args) {
    AddSkuReq addSkuReq = new AddSkuReq();
    addSkuReq.setColor("123");
    TProdSku tProdSku = new TProdSku();
    BeanUtils.copyProperties(addSkuReq,tProdSku);
    System.out.println(tProdSku.getColor());
  }
}
